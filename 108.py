...
def trade(context):
    ...
    upserise.columns=['upserise']
    downseries.columns=['downseries']
    ...
    for value in holding:
        if value in decison.index and decison.loc[value,'spread']<0 or upserise.loc[value,'upserise']<0 or downseries.loc[value,'downseries']>0:
            order_target(value, 0)
    for value in stocklist.index:
        if stocklist.loc[value,'spread']>0 and g.waitdays<0 and enter_trend(value,5) and upserise.loc[value,'upserise']>0 or downseries.loc[value,'downseries']<0:
