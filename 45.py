...
def ffb_wlx(context):
    ...
    for value in context.portfolios:
        ...
        try:
            numerator = history(g.n, unit='1d', field='close', security_list=value, df=True, skip_paused=False, fq='pre')[:-1].mean()
            denominator =history(g.m, unit='1d', field='close', security_list=value, df=True, skip_paused=False, fq='pre')[:-1].mean()
