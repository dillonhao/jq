...
def trade(context):
    ...
    stocklist = decison.sort_values(by=['spread'],ascending=False).head(5)
    ...
    for value in holding:
        if value in decison.index and decison.loc[value,'spread']<0:
            order_target(value, 0)
    for value in stocklist.index:
        if stocklist.loc[value,'spread']>0 and g.waitdays<0 and enter_trend(value,5):
            order_target_value(value, 100000)
...
def stop_retreat(context,nday=10,retreat=0.1):
    for stock in context.portfolio.positions:
        prices=history(nday, unit='1d', field='close', security_list=instrument, df=True, skip_paused=False, fq='pre')
