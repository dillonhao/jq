...
def initialize(context):
    ...
    g.std=0.1 #std is bad for the wlx
    g.stddays=12  #nunber of day to lookback
...
def wlx_rule(context,stock):
    ...
    upserise=aa[1]
    downseries=aa[0]
    upseries=pd.rolling_mean(upseries,5)
    downseries=pd.rolling_mean(downseries,5)
    upserise=upserise-0.01
...
def enter_std(instrument):
    price=history(g.stddays, unit='5d', field='close', security_list=instrument, df=True, skip_paused=False, fq='pre')
    if price[instrument].pct_change().std()  > g.std:
        return False
    else:
        return True
